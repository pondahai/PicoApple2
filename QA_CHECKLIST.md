...
- [ ] **實體鍵盤掃描**：
  - 在 BASIC 提示字元下快速連續打字，確認沒有漏字或連鍵（Debounce 正常）。
  - 按下 Caps Lock / Shift 等組合鍵行為正確。
- [ ] **模擬搖桿 (Joystick via Keyboard)**：
  - 確認方向鍵（上、下、左、右）映射至 Paddle 0/1 正確。
  - **長按測試**：死死按住任一方向鍵，確認遊戲內角色移動**完全連續，沒有任何斷續感或卡頓**（需在 VBLANK 乾淨環境下掃描）。
  - 確認搖桿按鈕 0/1（預設為按鍵 A/B 或 PageUp/PageDown）觸發正常。
- [ ] **系統快捷鍵**：
  - [ ] `Fn + 1`：觸發 Warm Reset (Ctrl-Reset)，遊戲/程式重新啟動。
  - [ ] `Fn + 2`：觸發 Cold Reset (Reboot)，系統徹底重置，重新載入 Track 0。
  - [ ] `Fn + 3`：成功呼叫出「磁碟選單 (Disk Menu)」。
  - [ ] `Fn + 4`：切換「搖桿模式 / 鍵盤模式」，畫面左下角有文字提示。

## 💾 4. 磁碟與 SD 卡 I/O (Disk & SD Card)
- [ ] **熱插拔與初始化**：開機時若未插卡，應有錯誤提示；插卡狀態下能正確讀取根目錄。
- [ ] **磁碟選單 (F3 Menu)**：
  - 開啟選單後，畫面渲染停止，僅顯示選單 UI。
  - **選單導航**：在選單內使用上下方向鍵可以正常捲動（此處的按鍵讀取不可失效）。
  - 按下確認鍵能成功載入 `.DSK` 檔案並自動退出選單。
  - 按下 `ESC` 鍵能取消並退出選單。
