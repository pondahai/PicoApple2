...
## 🔁 壓縮檔回壓時機:加入「馬達停轉防抖」自動回壓
原本回壓(WORK → 原 `.gz`/`.zip`)只在 **F3 換片**時觸發,`SAVE` 後不換片就不會存回壓縮檔。
改為**馬達持續停轉一段時間就自動回壓**:

- 馬達停轉([`loop()`])→ `flushDirtyTrack()`(落地工作檔)+ 記 `g_motor_off_time = millis()`。
- 馬達又轉起來 → `g_motor_off_time = 0`(取消待回壓,避開多步操作中途)。
- 主迴圈:`g_archive_dirty && !motor_on && 停轉 > ARCHIVE_REPACK_DELAY_MS(2500ms)` → `repackArchiveIfDirty()` 一次,並清 `g_motor_off_time`。
- 防抖效果:把一連串寫入(如 DOS `SAVE` 跨多軌、馬達中途短暫停轉)**合併成單次重壓**;
  純讀取不設 dirty 故不觸發。換片時的回壓保留當保險(已壓過則 `g_archive_dirty=false` 跳過)。
- 體感:核心本有 ~1s 馬達延遲停轉,故回壓約落在最後存取後 (1 + 2.5) 秒;回壓同步執行會
  短暫凍結模擬(gz 重壓 140KB,數百 ms~1s+),在 watchdog(8s)內。

## 已知限制 (壓縮磁碟 v1)
- **回壓時機**:馬達停轉防抖(~3.5s)自動回壓 + F3 換片回壓。**純斷電**(寫入後馬達還沒
  停轉滿延遲就斷電)仍不會回壓 → 改動留在 WORK(不刪,資料不失),原壓縮檔未更新。
- **zip 寫回不壓縮**(stored)。
...
## 🔁 待辦:repack 機制重新設計(待下次討論)
使用者想改 repack 做法,下次繼續時再一起設計更好的方法。現況作為起點:
- 壓縮檔載入 → 解壓到暫存工作檔 `/_WORK.DSK`,引擎跑工作檔。
- `LASTDISK.TXT` 記**原壓縮檔路徑/檔名**(非工作檔);開機重新解壓到 WORK。
- 回壓時機:馬達停轉防抖 ~2.5s + F3 換片;做法是整檔重壓(`/_REPACK.TMP` → rename)。
- 待討論方向(未定):避免每次 SAVE 整檔重壓、斷電安全、工作檔重用/保留、是否改以工作檔狀態為準等。
- 詳見 PROGRESS.md「近期計畫」。
