...
本專案支援 **實體鍵盤矩陣** 與 **虛擬終端機** 兩種輸入方式。大部分的操作在兩者間是共通的，主要差異在於**系統功能鍵**的觸發方式（終端機直接使用 `F1~F5` / `F7` 鍵，而實體鍵盤使用 `Fn + 數字` 組合鍵）。
...
### 5. Shift 符號排列 (現代排法，非 Apple II+ 原版)

`keymap_shift` 的數字列**刻意採用現代 PC 鍵盤排列** —— 這台機器裝的是現代鍵帽，鍵帽印什麼就送什麼。真正的 Apple II+ 排法不同，數字列有六處差異：

| 鍵 | Apple II+ 原版 | 本專案 (現代) |
| :--- | :--- | :--- |
| `Shift + 1` | `!` | `!` |
| `Shift + 2` | `"` | **`@`** |
| `Shift + 3` | `#` | `#` |
...
**為什麼原版是那樣**：Apple II+ 用的是 **bit-paired**（位元配對）鍵盤，源自 ASR-33 電傳打字機。Shift 的規則很機械 —— 把字元的 ASCII **XOR `0x10`**：
...
同一條規則也解釋了 Apple II 上那幾個有名的怪組合：`Shift + P` = `@`（`0x50^0x10=0x40`）、`Shift + N` = `^`、`Shift + M` = `]`。現代鍵盤則是 typewriter-paired，符號位置照打字機習慣排，與 ASCII 碼值無關。

**實務差異**：在真 Apple II+ 上輸入字串的雙引號要按 `Shift + 2`，本專案要按 `Shift + '`。若你在跑的老程式或書上教材是照原版鍵位寫的，符號鍵請對照上表。

> 這是**設計決定，不是 bug**。記錄於此是為了避免日後看到 `Shift + 2` 送出 `@` 時，誤以為 `keymap_shift` 轉換表壞掉而重新追查一次。實體鍵盤矩陣與轉換表的對應驗證，見姊妹專案 [PicoApple2-KeyboardTester](https://github.com/pondahai/PicoApple2-KeyboardTester)。
