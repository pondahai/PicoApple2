...
## 📅 近期計畫 (Next Steps)
0.  **🔁 壓縮檔 repack 機制重新設計 (待討論)**: 使用者想改 repack 做法,下次繼續時一起討論出更好的方法。
    - **目前機制(作為討論起點)**:
      - 載入壓縮檔 → 解壓到暫存工作檔 `/_WORK.DSK`,引擎跑工作檔。
      - `LASTDISK.TXT` 記錄的是**原壓縮檔的路徑/檔名**(非工作檔);開機 `openLastDisk()` 讀它 → 重新解壓到 WORK。
      - 回壓(WORK → 原 `.gz`/`.zip`)時機:**馬達停轉防抖 ~2.5s 自動回壓** + **F3 換片回壓**(保險)。
      - 回壓 = 整檔重壓覆蓋原檔(暫存 `/_REPACK.TMP` → `SD.rename`)。
    - **待討論點(方向未定)**:何時/如何回壓更好(例如避免每次 SAVE 都整檔重壓、斷電安全、工作檔重用/保留策略、是否改記工作檔狀態等)。先記著,下次設計。
