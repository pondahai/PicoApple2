...
### 外接鍵盤 F 鍵映射的兩項落差（同日修正，實機驗證通過 ✅）

1.  **F 鍵差一格**：原本韌體把 `[15~`（實體 **F5**）對到事件 4、`[17~`（實體 **F6**）對到 5。
    網頁是照這個錯位寫的（按 F4 送 `[15~`），兩邊一起錯所以網頁端表現正確；
    但第三方序列終端（PuTTY 等）照標準送，按 F4 的 `[14~` 沒人認 → 沒反應，
    F5/F6 則變成切搖桿/切速度，與畫面上的速查列對不起來。
    **修正**：韌體改收正規序列 `[14~`=F4、`[15~`=F5、`[18~`=F7，網頁同步改送 `[14~`/`[15~`。
    `[17~`（F6）沒有對應功能，留空不接 —— 速查列上本來就沒有 F6。
    順帶修掉一個從來沒生效過的死碼：`strcmp(esc_buf, "OS")` 寫在 `esc_buf[0] == '['` 的分支裡，
    永遠不可能成立；真正處理 SS3 的是下面的 `'O'` 分支，該分支原本只認 P/Q/R，現補上 S(F4)。
2.  **`'K'` 封包會漏字元**：網頁協定的 `'K'` 路徑原本只認 idx 112/113/114（F1~F3 的 JS keyCode），
    其餘一律 `pushKey(idx)` —— 客戶端送 keyCode 115（F4）會被當成 ASCII 憑空打出一個 `'s'`。
    **修正**：112~123（F1~F12）整段吃掉，其中 112~116 對應 F1~F5、118 對應 F7，
    F6 與 F8~F12 無功能也一併吞掉不送。

> ⚠️ 這兩項同樣是**兩端一起改**，韌體要重燒，網頁要重載。
